...
public class Monster2move : MonoBehaviour
{
    ...
    //结束控制
    public int monster_actnum;//怪物出现声音
    //
    ...
    void monstermove()
    {
                ...
                if (s > 0.1f)
                {
                    ...
                    if (s > disance)
                    {
                        ...
                        { //transform.localRotation = Quaternion.Euler(0, 180, 0);
                            transform.localEulerAngles = new Vector3(0, 180, 0);//旋转自己坐标方向
        ...
        else
        {
            ...
            stopbossskillnum(skillnum);//清楚boss生命恢复携程 消灭诈尸情况
    ...
    IEnumerator bossskill1()//守底boss 1鸡公
    {
    ...
    IEnumerator bossskill2()//守底boss 猴子
    {
        if (treasure_bool == true && this.monster1health > 0)
        {
            for (; ; )
